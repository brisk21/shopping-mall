...
class Account extends Controller
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        $this->defaultUrl = URL_WEB . 'mall/home/index';
        $this->param = input();


        if (!empty($this->param['bs_redirect_uri'])) {
            //缓存来源
            cookie($this->keyBackTo, urldecode($this->param['bs_redirect_uri']));
            $url = URL_HTTP . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
            $url = remove_query_var($url, 'bs_redirect_uri');
            return $this->redirect($url);
        }
        $sessionUid = CommonUser::session_get();
        if (!empty($sessionUid)) {
            $this->uid = $sessionUid;
        }
    }
    ...
    {
        return redirect('/console/account/login');
    }
